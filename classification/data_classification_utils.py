...
def simple_image_featurization(image):
    ...
    featureVec = np.array([pixelList])
    featureVec = featureVec.flatten("F")
    return featureVec
...
def zero_one_loss(classifier, samples, labels):
    ...
    for i in range(0, len(samples) - 1):
        classified = classifier.classify(samples[i])
        if classified != labels[i]:
            correct += 1.0
    return 1.0 - correct/len(samples)
