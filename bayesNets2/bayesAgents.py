...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            #populate domains with obs_vals
            variableDomainsDict[obsVar] = OBS_VALS
    #populate with remaining 4 edges
    ...
    #set variableDomainsDict[var] to values
...
def fillObsCPT(bayesNet, gameState):
    ...
    locations = {bottomLeftPos: BOTTOM_LEFT_VAL, topLeftPos: TOP_LEFT_VAL, bottomRightPos: BOTTOM_RIGHT_VAL, topRightPos: TOP_RIGHT_VAL}

    #iterate over all house positions
    for housePos in gameState.getPossibleHouses():
        location = locations[housePos]
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for dicts in obsFactor.getAllPossibleAssignmentDicts():
                #prob for food 
                if dicts[FOOD_HOUSE_VAR] == location:
                    if dicts[obsVar] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif dicts[obsVar] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                    else:
                        prob = 0
                #prob for ghost
                elif dicts[GHOST_HOUSE_VAR] == location:
                    if dicts[obsVar] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif dicts[obsVar] == BLUE_OBS_VAL:
                ...
                #no ghost or food
                    ...
                    if dicts[obsVar] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0
                obsFactor.setProbability(dicts, prob)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    factor = inference.inferenceByVariableElimination(bayesNet, ['FoodHouse'], evidence, eliminationOrder)
    prob_dist = -float('inf')
    best_dict = dict()
    for assignment_dict in factor.getAllPossibleAssignmentDicts():
        if factor.getProbability(assignment_dict) > prob_dist:
            prob_dist = factor.getProbability(assignment_dict)
            best_dict = assignment_dict
    return best_dict
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        cptable = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        foodtopLeft = dict(evidence)
        foodtopLeft[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        foodtopLeft[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL

        foodtopRight = dict(evidence)
        foodtopRight[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        foodtopRight[GHOST_HOUSE_VAR] = TOP_LEFT_VAL

        leftExpectedValue = cptable.getProbability(foodtopLeft) * WON_GAME_REWARD + (1-cptable.getProbability(foodtopLeft)) * GHOST_COLLISION_REWARD
        rightExpectedValue = cptable.getProbability(foodtopRight) * WON_GAME_REWARD + (1 -cptable.getProbability(foodtopRight)) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for prob, expEvidence in self.getExplorationProbsAndOutcomes(evidence):
            expectedValue += prob * max(self.computeEnterValues(expEvidence, enterEliminationOrder))
